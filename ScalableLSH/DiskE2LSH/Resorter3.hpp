...
public:
   ...
   * Multicore reranking
   */
  void static resort_multicore(long long int queryidx,
    ...
    cout << "NORMALIZE_FEATS: " << NORMALIZE_FEATS << endl;
    vector<float> qfeat;
    if (!feats->Get((queryidx + 1) * 10000 + 1, qfeat)) {
        cout << "Wrong " << queryidx << endl;
    } else {
      #if NORMALIZE_FEATS == 1
        L2Normalize(qfeat);
      #endif
      ...
      for (int i = 0; i < nMatches; i++) {
        res.push_back(make_pair(scores[i], i + 1));
      }
