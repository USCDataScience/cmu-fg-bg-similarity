...
class DiskMapLMDB {
  ...
  public:
  DiskMapLMDB(fs::path _fpath, bool _rdonly = false) : 
  ...
  bool Get(const Tkey& pos, Tval& output) const {
    output.clear();
    MDB_val key, data;
    string pos_s;
    hashObj(pos, pos_s);
    ...
    if (rc == MDB_NOTFOUND) {
      cerr << "Unable to read element" << endl;
      return false;
    }
    ...
    unhashObj(output, str);
    delete[] cstr;
    return true;
  }

  bool Put(const Tkey& pos, const Tval& input) {
    string hash;
    hashObj(input, hash);
